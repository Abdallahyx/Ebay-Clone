...
MESSAGES = {
    "token_miss_error": ("This token does not exist or belongs " "to another user!"),
    ...
    "registration_mail_sent": (
        "Mail with registration link has " "been sent to your email."
    ),
    "email_changing_sent": (
        "Mail with email changing confirmation "
        "has been sent to your new email. "
        "Your email in this account will be "
        "changed after confirmation."
    ),
    "password_reset_sent": (
        "Mail with password reset confirmation has been sent " "to your email."
    ),
}
...
class AuthTokenMixin(MailContextMixin):
    ...
    def _create_token(self, email: str) -> TokenData:
        ...
        if not UserToken.objects.filter(
            token_owner=email, token_type=self.token_type
        ).exists():
...
def get_token_data(token: str, email: str) -> TokenData:
    ...
    """
    try:
        token = UserToken.objects.get(token=token, token_owner=email)
        if token.expired:
